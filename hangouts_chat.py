...
from errbot.backends.base import Card, Message, Person, Room, RoomError
...
class MalformedCardError(Exception):
    def __init__(self, card: Card, message: str):
        super().__init__(("Malformed or unexpected card data provided:\n" f"  {message}"))
...
class GoogleHangoutsChatAPI:
    ...
    def _request(self, uri: str, query_string: str = None, **kwargs) -> Optional[dict]:
        ...
        result, content = self.client.request(uri=url, **request_args)
    ...
    def _download(self, uri: str) -> Optional[bytes]:
        ...
        result, content = self.client.request(uri=uri, **request_args)
        if result["status"] == "200":
            return content
        else:
            log.error("status: {}, content: {}".format(result["status"], content))
    ...
    def get_members(self, space_name: str) -> Iterable[dict]:
        return self._list(
            "spaces/{}/members".format(removeprefix(space_name, "spaces/")), "memberships"
        )

    def get_member(self, space_name: str, name: str) -> Optional[dict]:
        return self._request(
            "spaces/{}/members/{}".format(removeprefix(space_name, "spaces/"), name)
        )
...
class HangoutsChatRoom(Room):
    ...
    def occupants(self):
        ...
        for membership in memberships:
            ...
            occupants.append(
                HangoutsChatUser(
                    name, membership["member"]["displayName"], None, membership["member"]["type"]
                )
            )
...
class GoogleHangoutsChatBackend(ErrBot):
    ...
    def _event_cache_format_key(self, event_data):
        ...
        message_last_update_time = event_data.get("message", {}).get(
            "lastUpdateTime", "NO_MESSAGE_LASTUPDATETIME_PROVIDED_BY_GHC"
        )
    ...
    def _handle_event(self, event):
        ...
        if event_type == "MESSAGE":
            ...
            log.warn(
                f"Event type '{event_type}' received, handling as 'MESSAGE' type to support previous backend implementation."
    ...
    def handle_event_CARD_CLICKED(self, data):
        ...
        if action_method_name == "bot_command":
            ...
            log.info(
                f"Unsupported CARD_CLICKED event action method name '{action_method_name}' received"
            )
    ...
    def handle_event_MESSAGE(self, data):
        ...
        sender = HangoutsChatUser(
            sender_blob.get("name", ""),
    ...
    def send_card(self, errbot_card: Card, space_id=None, thread_id=None):
        if not isinstance(errbot_card, Card):
            log.warning(
                "deprecated signature of 'send_card' method called, recommend changing to current version that matches upstream signature."
            )
        ...
        if errbot_card.link:
            raise MalformedCardError(
                errbot_card, "'link' field not supported, please use body field."
            )
        if errbot_card.fields:
            raise MalformedCardError(
                errbot_card, "'fields' field not supported, please use body field."
            )
        if errbot_card.image:
            raise MalformedCardError(
                errbot_card, "'image' field not supported, please use body field."
            )
        ...
        message_payload = {"cards": [ghc_card]}
    ...
    def serve_forever(self):
        subscription = self._subscribe_to_pubsub_topic(
            self.gce_project, self.gce_topic, self.gce_subscription, self._handle_event
        )
